...
namespace MyLetterManager
{
    public static class LetterManager
    {
        ...
        static public string _adress_type = "Прописка";
        ...
        {
            string query = "";
            HeaderBuilder(ref query, command);
            ConditionsBuilder(ref query);
            return query;
        }

        static void HeaderBuilder(ref string query, string command)
        {
            switch (command)
            {
                case "select":
        ...
        static void ConditionsBuilder(ref string query)
        {
